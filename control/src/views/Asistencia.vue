...
async function reconocer() {
  ...
  setInterval(async () => {
    ...
    try {
      ...
      try {
        ...
        if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
           estado.value = "❌ ERROR: Memoria llena. No se guardó el registro.";
        } else {
           estado.value = "❌ Error al guardar datos";
        }
        detectando = false; // Release lock even on error
      }
    ...
    // Note: detectando is reset in setTimeout for success case, OR in error catch above.
    // We removed the generic reset at the bottom to avoid race condition with the success timeout
  }, 500);
}
...
onMounted(async () => {
  try {
    ...
    // Wait for video only for recognition
    if (video.value) {
        video.value.onplay = () => {
            reconocer();
        };
    ...
    console.error(err);
    estado.value = "❌ Error al iniciar: " + err.message;
  }
});
</script>
