...
function guardarPersona() {
  ...
  if (personas.value.length > 0) {
    const match = personas.value.find(p => {
        if (!p.descriptor) return false;
        const dist = faceapi.euclideanDistance(descriptorActual, p.descriptor);
        return dist < 0.5; // Threshold for similarity (lower is more similar)
    });

    if (match) {
        alert(`❌ Esta persona ya está registrada como: ${match.nombre} (ID: ${match.id})`);
        estado.value = "⚠️ Persona ya registrada";
        return;
    }
  }
